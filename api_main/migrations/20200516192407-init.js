"use strict"
//const { TABLES } = require("../contracts/db");
const sql = (strs, ...substs) => {
...
exports.up = async db => {
  ...
  try {
    ...
    let sqlQuery = sql`
      ...
      CREATE EXTENSION IF NOT EXISTS "uuid-ossp" SCHEMA "z_tasks";
    `
    ...
    sqlQuery = sql`
      CREATE TABLE ${TABLES.INVITE_TO_BOARD} (
        ...
        state VARCHAR (63) DEFAULT 'NEW' NOT NULL,
        description TEXT DEFAULT '',
...
exports.down = async db => {
  for (let table of Object.values(TABLES)) {
    let sqlQuery = `DROP TABLE IF EXISTS ${table} CASCADE;`
    await db.runSql(sqlQuery)
  }
